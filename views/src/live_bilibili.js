...
async function goListenBilibili(value, callback) {
    if (!value) return;
    openLive = [];
    sendText = [];
    const socketData = await getWebSocketHost(value);
    if (socketData.code) {
        console.log("房间号错误");
        return;
    }
    openSocket(socketData.data.host_list[0], value, callback);
}
...
function openSocket(url, room_id, callback) {
    let timer = null;
